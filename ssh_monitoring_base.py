    ...
    # Slack webhook
    SLACK_WEBHOOK_URL2 = "https://hooks.slack.com/services/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"
    ...
    # Remplace la ligne cmd_pattern par :
    #pour les autre system qui ne marche pas avec le code en haute desactiver ceette commentaire
    #debut
    #cmd_pattern = re.compile(
    #    r"SSH user=(\w+) ip=([^\s]+).*tty=([^\s]+) cmd=\s*\d+\s+([^\r\n]+)\s+rc=(-?\d+)"
    #)
    ...
    cmd_pattern = re.compile(
        r"SSH user=(\w+) ip=([^\s]+).*?(\d+\.\d+\.\d+\.\d+).*tty=([^\s]+).*cmd=\s*\d+\s+([^\r\n]+)\s+rc=(-?\d+)"
    )
    #fin
    # === Fonctions ===
    ...
    def send_slack(message):
        try:
            ...
            requests.post(SLACK_WEBHOOK_URL2, json=payload, timeout=5)
        except Exception as e:
    ...
    notify(
                        "Commande exécutée",
                        f"\n| Utilisateur\t| IP_client\t| IP_serveur_cible\t| tty\t| rc\t| cmd\n"
